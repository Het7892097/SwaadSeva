...
const OrderPage = () => {
  ...
  const addToCart = (item) => {
    ...
    setCart((prevCart) => {
      ...
      if (existingItemIndex !== -1) {
        ...
        // console.log("Cart after updating quantity:", updatedCart); for checking the proper functinality of cart
        return updatedCart; // Return the new array
      } else {
        // Add new item with name, quantity 1, and parsed price
        const newCart = [
          ...prevCart,
          { name: item.name, quantity: 1, price: parsedPrice },
        ];
        return newCart;
      }
    });
  ...
  const currentProducts =
    productList.find((category) => category._id === selectedCategory)
      ?.products || [];
